...
// in sources.json) whose GRADED picks clear the promotion gate â€” a real sample AND a positive ROI even at
// the lower confidence bound (edgeLcb > 0) â€” earns a permanent spot in sources.json, so it is scanned every
// collect thereafter. It is GRADE-driven, never frequency-driven: a channel the search keeps finding but
// that never grades out is NOT promoted (origins-not-voices â€” no amplifiers earn a spot on volume).
//
...
// first). Today every graded source is already on the roster, so it is a no-op â€” by design, not a bug.
require("./lib/env");
const fs = require("fs");
...
(async () => {
  ...
  if (!graded) { say("run-promote-channels: no sources_graded.json yet â€” nothing to promote."); return 0; }
  ...
  if (!promotable.length) { say(`run-promote-channels: 0 channels clear the gate (n>=${MIN_N}, edgeLcb>0, non-roster). Nothing to promote${APPLY ? "" : " (dry run)"}.`); return 0; }
  ...
  for (const p of promotable) {
    ...
    say(`  ${p.source}  n=${p.n} edgeLcb=${p.edgeLcb} hitRate=${p.hitRate}  handle=${handle || "(unresolved â€” add manually)"}`);
    if (handle) toAdd.push({ name: p.source, domain: p.domain, type: p.type, platform: "youtube", handle, note: `auto-promoted: graded n=${p.n}, edgeLcb=${p.edgeLcb}`, trusted: false });
  }

  if (!APPLY) { say(`(dry run â€” re-run with --apply to append ${toAdd.length} channel(s) with a resolved handle to sources.json)`); return 0; }
  if (!toAdd.length) { say("nothing had a resolvable handle â€” added 0 (resolve those handles manually if you want them)."); return 0; }
  sources.push(...toAdd);
