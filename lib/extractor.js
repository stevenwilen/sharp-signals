...
async function callGeminiInner(system, user) {
  ...
  // flash-lite: ~3x cheaper than flash and it passed every direction trap (test/test-model.js).
  // Direction is the only thing that can poison a track record, and it gets it right, so the
  // cheaper model is the correct default on a fixed budget. Override with EXTRACT_MODEL.
  ...
  for (let attempt = 0; attempt < 8; attempt++) {
    ...
    if (r.status === 429 || r.status >= 500 || r.status === 0) {
      lastErr = r.status === 429 ? "rate-limited" : `http ${r.status}${r.err ? " " + r.err : ""}`;
