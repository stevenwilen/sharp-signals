...
function topicsFor(claims) {
  ...
  return [...groups.values()].map((g) => {
    ...
    return {
      ...
      // the source record, never discarded
...
function evaluateBout(bout, claims) {
  ...
  for (const t of topics) {
    ...
    if (t.origin.independentOrigins === 1 && t.marketAwareness === "newly_emerging" && t.strength !== "very_weak")
      reviewItems.push({ why: "potentially new information with a single origin", topic: t.topic, about: t.about, example: t.claims[0].claim });
    ...
    if (ent.needsHumanReview || ent.confidence === "low")
      reviewItems.push({ why: "ambiguous fighter identity", topic: t.topic, about: t.about, resolution: ent });
  }
