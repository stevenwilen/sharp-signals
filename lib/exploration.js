...
// judgement.
// opts.scale = live bankroll / the $100 calibration base, so tier stakes track the ACTUAL balance without
// touching the frozen tier fractions/dollars (scale 1 = the original $100 behaviour, used everywhere in tests).
function classifyAndSize(valued, exploration, opts = {}) {
  ...
  // shot drains the bankroll fast. Scale the stake by the price (≈ win probability): full tier stake at even
  // money (fullStakeAtOrAbove) or better, proportionally less below it. Conviction still picks the fight
  // (the tier), the odds size the bet. priceScaled:false reverts to flat tiers.
  const SZ = RULES.sizing || {};
  const price = valued.allInPrice != null ? valued.allInPrice : valued.topOfBookPrice;
  const stakeFactor = (SZ.priceScaled && price > 0)
    ? Math.min(1, price / (SZ.fullStakeAtOrAbove || 0.5))
    : 1;
  ...
  for (const tier of ["BEST EXPERIMENTAL", "STRONG SPECULATIVE", "CREATIVE SPECULATIVE"]) {
    if (meets(tier)) {
      return {
        classification: tier, tier, stake: +(T[tier].stake * scale * stakeFactor).toFixed(2), fraction: T[tier].fraction,
...
// Apply per-fight (5%) and per-card (10%) exposure caps. opts.scale tracks the live bankroll vs the $100
// base, so the caps scale with the balance too (scale 1 = the original $5 / $10 dollar caps).
function applyExposureCaps(positions, opts = {}) {
  const scale = opts.scale > 0 ? opts.scale : 1;
  const capF = RULES.caps_exposure.maxPerFightDollars * scale, capC = RULES.caps_exposure.maxPerCardDollars * scale;
  let cardTotal = 0;
