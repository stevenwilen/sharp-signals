...
// A minimum gap throttles the cache-MISS burst (the first fetch of each new fighter) so the deep
// run doesn't hammer BFO; cache hits skip it entirely.
let lastFetch = 0;
...
async function fetchCached(url) {
  ...
  if (gap < FETCH_GAP_MS) await new Promise((r) => setTimeout(r, FETCH_GAP_MS - gap));
  lastFetch = Date.now();
...
// LIVE fetch: deliberately NOT cached. fetchCached is right for settled fights ("pages never
// change") and fatal for live odds — a cached page hands back yesterday's price and every
// comparison silently becomes a lie. But fetchText has no throttle of its own, so a burst of live
// fetches is how you get blocked: gap them here.
let lastLive = 0;
const LIVE_GAP_MS = 700;
async function fetchLive(url) {
  const gap = Date.now() - lastLive;
  if (gap < LIVE_GAP_MS) await new Promise((r) => setTimeout(r, LIVE_GAP_MS - gap));
  lastLive = Date.now();
  return fetchText(url);
}

// The CURRENT de-vigged probability for `pick` vs `opponent` in a fight around `fightDateMs`.
// Same page + parse as closingLine, but for an UPCOMING fight BFO's three moneylines are
// [open, current-low, current-high] — a live range across books — so the midpoint is today's
// consensus rather than a closing line. Same overround sanity gate: a parse that grabbed the wrong
// cells produces a wild overround and is rejected rather than believed.
//
// Returns a REASON on failure instead of null. The first cut of the divergence test swallowed its
// exceptions and reported 9 "no BFO match" that were really one date bug — silent failure, the
// house speciality. Callers log the reason.
async function liveLine(pick, opponent, fightDateMs) {
  const tryPage = async (who, vs) => {
    let cands;
    try { cands = await resolveCandidates(who); }
    catch (e) { return { err: `search failed: ${e.message}` }; }
    if (!cands.length) return { err: "name not found on BFO" };
    let err = "no BFO fight row near that date";
    for (const c of cands.slice(0, 2)) {
      let html;
      try { html = await fetchLive(`https://${HOST}${c.path}`); }
      catch (e) { err = `fetch failed: ${e.message}`; continue; }
      const hit = pickFight(parseHistory(html), who, vs, fightDateMs);
      if (hit) return { hit };
    }
    return { err };
  };

  // Try the pick's own page, then fall back to the OPPONENT's page (the same fight is listed there
  // too, sides swapped). Kalshi's name for a fighter routinely differs from BFO's — "Christian
  // Duncan" vs "Christian Leroy Duncan" — so one page resolving and the other not is normal, and
  // without this fallback a perfectly available fight reads as missing data.
  const first = await tryPage(pick, opponent);
  let hit = first.hit, swapped = false;
  if (!hit && opponent) {
    const second = await tryPage(opponent, pick);
    if (second.hit) { hit = second.hit; swapped = true; }
  }
  if (!hit) return { ok: false, reason: first.err || "no BFO fight row" };

  const me = swapped ? hit.opp : hit.me;   // on the opponent's page, WE are the opp side
  const op = swapped ? hit.me : hit.opp;
  const dv = deVigProbs(me.closeProb, op.closeProb);
  if (!dv) return { ok: false, reason: "could not de-vig (missing lines)" };
  ...
  return {
    ok: true, prob: +dv.probA.toFixed(4), oppProb: +dv.probB.toFixed(4),
    overround: +dv.overround.toFixed(4), fighter: me.name, opponent: op.name,
    date: hit.ft.date, event: hit.ft.event,
  };
}
...
module.exports = {
  fetchText, fetchCached, fetchLive, mlToProb, deVigProbs, parseHistory,
  resolveCandidates, resolveFighter, pickFight, lookup, closingLine, liveLine, HOST, CACHE_DIR,
};
