...
const PAPER_ELIGIBLE_CATEGORIES = Object.freeze(["CONFIRMED_SYSTEM_BET"]);
...
function load() {
  try { const j = JSON.parse(fs.readFileSync(FILE, "utf8")); return { bankroll: j.bankroll || BANKROLL, entries: j.entries || {}, meta: j.meta || {}, cashAdjustments: j.cashAdjustments || [] }; }
  catch (e) { if (e.code === "ENOENT") return { bankroll: BANKROLL, entries: {}, meta: {}, cashAdjustments: [] }; throw e; }
}
function save(state) {
  const tmp = FILE + ".tmp";
...
function recordDiscretionary(state, d = {}) {
  ...
  const entry = stampCategory({
    ...
    lane: "discretionary",
    classification: d.classification || "DISCRETIONARY",
    kind: "single",
...
function summary(state) {
  ...
  const netCashAdjustments = r2((s.cashAdjustments || []).reduce((a, x) => a + (Number(x.amount) || 0), 0)); // external deposits (+) / withdrawals (−)
  const availableCash = r2(starting + realizedPnl + netCashAdjustments - openExposure - feesActual);        // per the formula
  ...
  return {
    ...
    realizedPnl, netCashAdjustments, openExposure, availableCash, accountValue, maxPendingPayout,
...
// External cash movement — a deposit (+) or withdrawal (−) that is NOT a bet. Keeps the ledger's balance
// equal to the REAL Kalshi account balance (e.g. money withdrawn to your bank). Never invents a value.
function recordCashAdjustment(state, { amount, note, at } = {}) {
  const amt = Number(amount);
  if (!Number.isFinite(amt) || amt === 0) throw new Error("cash adjustment needs a nonzero amount (+ deposit / − withdrawal)");
  (state.cashAdjustments = state.cashAdjustments || []).push({ amount: r2(amt), note: note || null, at: at || nowIso() });
  return state;
}

module.exports = {
  STATUS, FILE, BANKROLL, load, save,
  recordRecommendation, confirmPlacement, declinePlacement, settle, recordDiscretionary, recordCashAdjustment,
