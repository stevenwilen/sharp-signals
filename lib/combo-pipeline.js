...
// Drop records for cards that have already STARTED. A combo is only live for an UPCOMING card, but
// gatherLegs stops building it the moment the card begins (line 42) — so nothing overwrites the record and
// it lingers on the dashboard as a stale "old combo bet." Prune by the legs' event date; if that's missing
// (older records), fall back to a record that hasn't been refreshed in 36h. Same 22:00 convention the rest
// of this file uses.
function pruneStaleRecords(audit, now) {
  let pruned = 0;
  for (const [id, rec] of Object.entries((audit && audit.records) || {})) {
    const dates = ((rec && rec.audit && rec.audit.legs) || []).map((l) => l && l.eventDate).filter(Boolean);
    const stale = dates.length
      ? dates.every((d) => FR.fightStarted(d, now))
      : (Number.isFinite(Date.parse(rec && rec.at)) && (now - Date.parse(rec.at)) > 36 * 3600e3);
    if (stale) { delete audit.records[id]; pruned++; }
  }
  return pruned;
}
...
async function runCombo(opts = {}) {
  ...
  const pruned = pruneStaleRecords(audit, now);   // clear out old-card combos before anything else
  const results = [];

  if (legs.length < 2) {
    if (pruned && opts.persist !== false) writeJson(AUDIT_FILE(), audit);   // persist the prune even on the early exit
    return { decision: CE.DECISION.NO_BET, reason: `fewer than 2 eligible BUY legs (${legs.length}) — no combo to analyze`, legs: legs.length, results, sent: [], pruned };
  }
...
module.exports = { runCombo, gatherLegs, conservativeProbFor, comboKey, enabled, AUDIT_FILE, pruneStaleRecords };
