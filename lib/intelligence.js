...
// ---- STORY IDENTITY (§ identity hardening) --------------------------------------------------------
// (event, fighter, topic, direction) was too coarse: a knee injury, a staph report, and a withdrawal
// about the same fighter all land in ONE (injury_health, against_about) topic and would merge into one
// record. The identity must key on the STORY — the specific proposition — so those stay three records,
// while paraphrases and amplifiers of ONE story keep updating one record.
//
// The proposition is a canonical signature: an EVENT class (the fight may not happen) dominates; else a
// set of CONDITION/site markers; else the topic+direction as a coarse fallback. Synonyms collapse
// ("withdrew" / "pulled out" / "off the card" → event:exit) so paraphrase ≠ new story; distinct
// subjects ("knee" vs "staph") stay apart.
const EVENT_MARKERS = [
  [/\b(withdraw|withdrew|withdrawn|pull(?:s|ed)? out|off the card|out of the (?:fight|card|bout)|fight is off|bout is off|scratched|cancel(?:l?ed|s|lation)?)\b/, "exit"],
  [/\b(replac(?:e|ed|ement)|steps? in|stepping in|new opponent|short.notice replacement)\b/, "replacement"],
  [/\b(suspend(?:ed|s|sion)?|usada|vada|failed (?:a )?(?:drug|ped) test|flagged for)\b/, "suspension"],
  [/\b(hospitali[sz]ed|in hospital|rushed to hospital)\b/, "hospital"],
  [/\bvisa\b/, "visa"],
];
const EVENT_RANK = ["exit", "replacement", "suspension", "hospital", "visa"];
const CONDITION_MARKERS = [
  [/\bstaph\b/, "staph"], [/\b(infection|infected)\b/, "infection"],
  [/\b(illness|ill\b|sick|flu|fever|covid|pneumonia)\b/, "illness"],
  [/\b(weight cut|missed weight|missing weight|drained|dehydrat|depleted|on the scale)\b/, "weightcut"],
  [/\b(concussion|head trauma|cte)\b/, "concussion"], [/\b(cut|laceration|stitches)\b/, "laceration"],
];
const INJURY_SITES = ["acl", "mcl", "meniscus", "knee", "shoulder", "elbow", "wrist", "hand", "thumb", "finger",
  "foot", "ankle", "shin", "calf", "hamstring", "quad", "groin", "hip", "rib", "ribs", "back", "neck", "jaw",
  "nose", "orbital", "eye", "achilles", "labrum", "bicep", "tricep", "toe", "tooth"];

// The canonical proposition signature for one claim's text. Event class wins; else conditions+sites;
// else topic:direction. This is what makes two paraphrases share an id and two injuries not.
function propositionSignature(text, topic, direction) {
  const t = norm(text);
  const ev = EVENT_MARKERS.filter(([re]) => re.test(t)).map(([, c]) => c);
  if (ev.length) return `event:${EVENT_RANK.find((r) => ev.includes(r)) || ev[0]}`;
  const cond = new Set();
  for (const [re, c] of CONDITION_MARKERS) if (re.test(t)) cond.add(c);
  for (const s of INJURY_SITES) if (new RegExp(`\\b${s}\\b`).test(t)) cond.add(s);
  if (cond.size) return `cond:${[...cond].sort().join("+")}`;
  return `topic:${topic}:${direction}`;
}

// A STABLE story id: same (event, fighter, proposition) → same record, across boutId renumbering,
// re-worded transcripts, and opponent replacement. This is the anti-duplication guarantee AND the
// anti-over-merge guarantee.
function storyId(eventId, about, signature) {
  const h = crypto.createHash("sha256").update(`${eventId}|${norm(about)}|${signature}`).digest("hex").slice(0, 12);
  return `intel_${h}`;
}
// Back-compat shim: the old topic-level key, still used where a whole topic is one story.
function stableId(eventId, about, topic, direction) {
  return storyId(eventId, about, `topic:${topic}:${direction}`);
}

// SPLIT one evaluated topic into its distinct STORIES. Claims are grouped by proposition signature. A
// single-story topic keeps the upstream origin count (computed over exactly these claims); a topic that
// splits into several stories has origins RE-COUNTED per story via the same originAnalysis — so a knee
// report and a staph report can never share an origin number.
function storyGroups(topic) {
  const groups = new Map();
  for (const claim of topic.claims || []) {
    const sig = propositionSignature(`${claim.claim || ""} ${claim.quote || ""}`, topic.topic, topic.direction);
    if (!groups.has(sig)) groups.set(sig, []);
    groups.get(sig).push(claim);
  }
  const entries = [...groups.entries()];
  if (entries.length <= 1) {
    const sig = entries.length ? entries[0][0] : propositionSignature(topicText(topic), topic.topic, topic.direction);
    return [{ ...topic, proposition: sig, propositionSignature: sig }];
  }
  return entries.map(([sig, claims]) => {
    const origin = E.originAnalysis(claims);
    const kinds = [...new Set(claims.map((c) => c.kind).filter(Boolean))];
    const relevance = [...new Set(claims.map((c) => c.relevance).filter(Boolean))];
    const freshness = [...new Set(claims.map((c) => c.freshness).filter(Boolean))];
    let strength = topic.strength;
    try { const cred = claims.map((c) => E.credibilityOf(c, claims, origin)); strength = cred.reduce((a, b) => (b._plus - b._minus > a._plus - a._minus ? b : a), cred[0]).strength; } catch (_) {}
    return {
      ...topic, claims, proposition: sig, propositionSignature: sig, origin,
      claimCount: claims.length,
      kinds: kinds.length ? kinds : topic.kinds,
      relevance: relevance.length ? relevance : topic.relevance,
      freshness: freshness.length ? freshness : topic.freshness,
      strength,
      marketAwareness: E.marketAwarenessOf(topic.topic, origin, claims),
    };
  });
}
...
function recordFromTopic(topic, ctx) {
  ...
  const o = topic.origin || {};
  const signature = topic.propositionSignature || propositionSignature(topicText(topic), topic.topic, topic.direction);
  const id = storyId(eventId, topic.about, signature);
  return {
    intelligenceId: id,
    ...
    normalizedClaim: `${norm(topic.about)} — ${signature}`,
...
function ingest(store, batch) {
  ...
  for (const bout of batch.bouts || []) {
    for (const topic of bout.topics || []) {
      // SPLIT the topic into its distinct stories first, so a knee report, a staph report and a
      // withdrawal about one fighter become three records, not one.
      for (const sub of storyGroups(topic)) {
        const sig = sub.propositionSignature;
        const topicKey = `${norm(sub.about)}|${topic.topic}`;
        // context may be keyed by the specific story (proposition) OR fall back to the whole topic.
        const opponent = (bout.opponentOf && bout.opponentOf[norm(sub.about)]) || null;
        const contradiction = (bout.contradictionByKey && (bout.contradictionByKey[sig] || bout.contradictionByKey[topicKey])) || null;
        const actionCtx = (bout.actionCtxByKey && (bout.actionCtxByKey[sig] || bout.actionCtxByKey[topicKey])) || {};
        const confirmed = bout.confirmedKeys && (bout.confirmedKeys.includes(sig) || bout.confirmedKeys.includes(topicKey));
        const disproved = bout.disprovedKeys && (bout.disprovedKeys.includes(sig) || bout.disprovedKeys.includes(topicKey));
        const ctx = {
          eventId: batch.eventId, boutId: bout.boutId, fight: bout.fight, opponent, now: batch.now,
          market: bout.market, contradiction, confirmed, disproved, ...actionCtx,
        };
        const id = storyId(batch.eventId, sub.about, sig);
        const prev = out.store.records[id];
        let record = prev ? mergeTopic(prev, sub, ctx) : recordFromTopic(sub, ctx);
        // keep the current boutId current even across renumbering / opponent replacement
        ...
        out.results.push({ id, proposition: sig, created: !prev, statusChanged: !prev || (prev.truthStatus !== applied.record.truthStatus),
...
module.exports = {
  ...
  stableId, storyId, propositionSignature, storyGroups,
  recordFromTopic, mergeTopic, classifyAction, applyAction, isMaterial,
