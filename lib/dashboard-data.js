...
const BK = require("./bankrolls");   // canonical two-ledger summary (Real Entertainment + Paper Strategy)
...
// SYSTEM HEALTH (certification observability). One view answering: is real, CURRENT information
// flowing — not merely "did workflows exit green". Reads persisted operational stores only.
function systemHealth(card) {
  const FR = require("./freshness");
  const CI = require("./candidate-index");
  const GL = require("./gemini-ledger");
  const disp = readPlain("dispatch-receipts.json");
  const ciStatus = CI.loadStatus();
  const sel = readPlain(`card-selection-${card}.json`);
  const fresh = (sel.ok && sel.data.freshness) || ciStatus || null;
  const corpusFreshness = fresh && (fresh.corpusFreshness || FR.corpusStatus({ newestSourceTs: fresh.newestSourceTs, eventDate: card }));
  let sourcesConfigured = null;
  try {
    const s = JSON.parse(fs.readFileSync(path.join(ROOT, "sources.json"), "utf8"));
    const arr = Array.isArray(s) ? s : (s.sources || s.channels || []);
    sourcesConfigured = Array.isArray(arr) ? arr.length : null;
  } catch (_) {}
  const gem = (() => { try { return GL.summary(); } catch (_) { return null; } })();
  return {
    activeCard: card,
    lastDispatchStages: disp.ok ? Object.fromEntries(Object.entries(disp.data).filter(([k]) => k !== "schemaVersion").map(([k, v]) => [k, v && v.ranAt])) : null,
    discovery: {
      // the LIVE candidate index — actual newest source timestamps, not workflow completion
      newestSourceTs: fresh ? fresh.newestSourceTs : null,
      corpusNewest: fresh ? fresh.corpusNewest : null,
      liveNewest: fresh ? fresh.liveNewest : null,
      livePicksFiles: fresh ? fresh.livePicksFiles : null,
      mergedCandidates: fresh ? fresh.merged : null,
      channelsInLive: fresh ? fresh.channelsInLive : null,
      channelsTotal: fresh ? fresh.channelsTotal : null,
      sourcesConfigured,
      indexBuiltAt: fresh ? fresh.builtAt : null,
      corpusFreshness: corpusFreshness || { status: FR.S.WAITING, reason: "no candidate index built yet" },
      liveUnreadable: fresh ? fresh.liveUnreadable : null,
      // the sensing pipeline's own health receipt: distinguishes "nothing new" from "the fetch failed"
      lastDiscovery: (() => { const d = readPlain("discovery-status.json"); return d.ok ? d.data : null; })(),
      kalshiFetchFailed: (() => { const r = readPlain("dispatch-receipts.json"); return (r.ok && r.data.kalshiFetchFailed) || null; })(),
    },
    modes: {
      production: productionEnabled(),
      exploration: process.env.EXPLORATION_ENABLED === "1",
      fightIntel: process.env.FIGHT_INTEL_ENABLED === "1",
      fightIntelSend: process.env.FIGHT_INTEL_SEND === "1",
      webResearch: process.env.INTEL_WEB_SEARCH === "1" ? "ENABLED" : "DISABLED",
      geminiModel: process.env.EXTRACT_MODEL || "gemini-flash-lite-latest",
    },
    gemini: gem,
    fightStarted: FR.fightStarted(card),
  };
}
...
function buildUnifiedDashboard(cardDate) {
  ...
  return {
    ...
    decisionSummary,
    exposure,
    // CANONICAL two-ledger summary — the SAME object the Sharp Signals app consumes (data/bankrolls.json),
    // so both boards show identical Real Entertainment ($100) and Paper Strategy ($10,000) totals. Neither
    // recomputes money independently. The manualBankroll block below is the detailed per-status view.
    bankrolls: (() => { try { return BK.build(); } catch { return null; } })(),
    ...
    bouts,
    systemHealth: systemHealth(card),
    ...
    convergence,
    // A meaningful lifecycle label, not a bare ABSENT: the grade/learning artifacts are legitimately
    // absent until a card has been fought and settled ("waiting for settlement"); a missing CORE
    // artifact is unexpected ("missing"). `state` is what the dashboard renders.
    artifactStatus: (() => {
      const st = (present, kind) => present ? "present" : (kind === "settlement" ? "waiting for settlement" : "missing");
      return [
        { name: "forecast", file: `forecast-${card}.json`, present: fcR.ok, reproduces: fcR.reproduces !== false, state: st(fcR.ok, "core") },
        { name: "entertainment-alerts", file: `entertainment-alerts-${card}.json`, present: alR.ok, state: st(alR.ok, "core") },
        { name: "attestation", file: "attestation.json", present: atR.ok, state: st(atR.ok, "core") },
        { name: "dispatch-receipts", file: "dispatch-receipts.json", present: dispR.ok, state: st(dispR.ok, "core") },
        { name: "evidence-eval", file: `evidence-eval-${card}.json`, present: evalR.ok, state: st(evalR.ok, "core") },
        { name: "learning-ledger", file: "learning-ledger.json", present: llR.ok, state: st(llR.ok, "settlement") },
        { name: "mechanism-reliability", file: "mechanism-reliability.json", present: mrR.ok, state: st(mrR.ok, "settlement") },
        { name: "convergence-eval", file: "convergence-eval.json", present: cvR.ok, state: st(cvR.ok, "settlement") },
      ];
    })(),
  };
}
...
module.exports = {
  ...
  buildUnifiedDashboard, latestForecastCard, fightIntelligenceView, intelDisplay, systemHealth,
