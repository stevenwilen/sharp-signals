...
function buildUnifiedDashboard(cardDate) {
  ...
  // VERIFICATION OVERLAY. A separate, additive record (data/verification-<card>.json) written when a
  // human runs down a review-queue item. It is overlaid onto the matching human-review alert for
  // DISPLAY ONLY — it never touches the sealed forecast, the origin counts, or any bet. A CONFIRMED
  // withdrawal whose market has already settled is exactly what belongs here: the reader should see
  // "bout withdrawn — market settled", not a stale "UNVERIFIED · 1 origin" that reads like live news.
  const verR = readPlain(`verification-${card}.json`);
  const verItems = verR.ok ? (Array.isArray(verR.data) ? verR.data : verR.data.items || []) : [];
  const vnorm = (s) => String(s || "").toLowerCase().trim();
  const verIndex = {};
  for (const v of verItems) {
    if (v.boutId) verIndex[`b|${v.boutId}|${vnorm(v.about)}|${vnorm(v.topic)}`] = v;
    verIndex[`a|${vnorm(v.about)}|${vnorm(v.topic)}`] = v;
  }
  const verificationFor = (h) =>
    (h.boutId && verIndex[`b|${h.boutId}|${vnorm(h.about)}|${vnorm(h.topic)}`]) ||
    verIndex[`a|${vnorm(h.about)}|${vnorm(h.topic)}`] || null;
  ...
  const bouts = (fc.forecasts || []).map((f) => {
    ...
    return {
      ...
      // A review item that has been verified (data/verification-<card>.json) no longer counts as
      // "unverified news" — so a confirmed-and-settled bout stops rendering the amber UNVERIFIED pill.
      verification: (() => {
        const checked = reviews.map(verificationFor).find(Boolean) || null;
        const unresolved = reviews.filter((h) => !verificationFor(h));
        return {
          unverifiedItems: unresolved.length,
          hasUnverifiedNews: unresolved.length > 0,
          contradictions: f.contradictions ?? null,
          boutWithdrawn: !!(checked && checked.status === "CONFIRMED_WITHDRAWN"),
          marketSettled: !!(checked && checked.marketState === "SETTLED"),
          checked,
        };
      })(),
      humanReview: reviews.map((h) => ({
        about: h.about, topic: h.topic, origins: h.origins, why: h.why, text: h.text,
        verification: verificationFor(h),
      })),
...
module.exports = {
  ...
  buildUnifiedDashboard, latestForecastCard,
