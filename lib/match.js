...
async function marketsFor(domain) {
  ...
  if (!series) return [];
  if (!cache[series]) {
    // Only MEMOIZE a real, non-empty board. An empty array (Kalshi returned a truncated/empty
    // 200) or a throw must NOT be cached as "the board" — otherwise `![]` is false, the empty
    // result sticks for the whole run, every pick sees "no market", and the pipeline settles the
    // entire live watch list. Leaving it uncached lets a later pick/run recover. Errors propagate.
    const r = await k.marketsAll({ series_ticker: series, status: "open" });
    if (Array.isArray(r) && r.length) cache[series] = r;
...
async function matchToMarket(pred, { now = new Date(), cfg = {} } = {}) {
  ...
  // Filler fights are not worth betting: protective matchmaking, a heavy favourite, nothing to
  // disagree about — sources are right constantly and earn nothing. A per-domain volume floor
  // filters them (see lib/cards.js). No MMA floor is configured today, so this currently passes all.
