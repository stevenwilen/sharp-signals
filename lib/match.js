...
const lastName = (s) => { const p = tokens(s); return p[p.length - 1] || ""; };

// How well does `text` refer to the fighter `name`?
//   3 = the text IS the name
//   2 = every token of the name appears in the text ("Kamaru Usman by KO" -> Kamaru Usman)
//   1 = only the surname appears  <- NOT trustworthy on its own; see below
//   0 = no reference
function nameScore(text, name) {
  ...
  if (nt.every((x) => tt.includes(x))) return 2;
...
async function matchToMarket(pred, { now = new Date() } = {}) {
