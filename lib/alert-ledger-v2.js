...
function shouldSend(key, state) {
  const all = load();
  let prev = all[key];
  // TICKER-STABLE FALLBACK for bout renumbering. Kalshi renumbers bouts as a card firms (B04->B05->B08),
  // so the SAME live contract's ledger key changes underneath us. An exact-key miss would then look like a
  // never-seen contract and re-fire `first` — a DUPLICATE 🟢 BUY for a bet already sent (the operator pays
  // the fee twice). When the exact key is absent, fall back to the most-recent prior entry for the SAME
  // ticker AND lane: dedup follows the contract's stable identity, not the volatile bout id. Because a
  // matched renumber then reports "no material change" and suppresses the send, record()/recordRecommendation
  // never run for it, so no duplicate ledger or real-money rows accumulate either — no migration needed.
  if (!prev && state && state.topTicker) {
    for (const [k, e] of Object.entries(all)) {
      if (k === key || k.startsWith("review|") || !e || e.topTicker !== state.topTicker) continue;
      if ((e.lane || null) !== (state.lane || null)) continue;   // never cross core <-> exploration on one ticker
      if (!prev || (e.lastSentAt || "") > (prev.lastSentAt || "")) prev = e;
    }
  }
  let fired = [];
  ...
  // Once we have ALREADY spoken about a contract, a re-seal / stake nudge / re-rank is not news the human
  // needs again — those are the HOLDER_TRIGGERS. The forecast re-seals EVERY cycle to capture fresh prices,
  // so its hash changes every ~10 min even when the pick, price and stake are identical; keying a re-send
  // on `forecast-changed` re-sent the SAME buy over and over (the duplicate the operator saw). Suppress the
  // holder-only triggers for ANY previously-sent contract — buy OR priced-out. The MATERIAL signals stay
  // outside this set and still fire: price crossed the ceiling, price is favourable again, or it withdrew.
  // Guarded on `prev` so a genuine first sighting (only `first` fires, not a holder trigger) is untouched.
  if (prev) {
