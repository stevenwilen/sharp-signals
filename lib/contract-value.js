...
function probabilityFor(contract, forecast, useExploration = false) {
  ...
  switch (contract.outcomeType) {
    case C.OUTCOME.FIGHTER_WINS: {
      // EXPLORATION lane: value the win against the CREATIVE central probability (core + capped
      // creative adjustment), kept separate from the frozen core. A is the forecast's fighter A, so
      // fighter A's creative win prob is creativeCentralA and B's is its complement. Only ever used
      // when the caller explicitly asks (opts.useExploration); core valuation is untouched.
      if (useExploration && forecast.exploration && forecast.exploration.creativeCentralA != null) {
        const cA = forecast.exploration.creativeCentralA;
        const p = E.norm(who) === E.norm(A) ? cA : +(1 - cA).toFixed(4);
        return { ok: true, probability: p, source: "exploration lane: core + capped creative adjustment", validated: true, lane: "exploration" };
      }
...
function valueContract(contract, forecast, orderbook, opts = {}) {
  ...
  const useExploration = opts.useExploration === true;
  const pm = probabilityFor(contract, forecast, useExploration);
  ...
  // An opinion is a core adjustment — OR, in the exploration lane, an active creative hypothesis.
  // The creative lane's whole purpose is to hold a view where core (correctly) holds none, so a bout
  // with a live creative hypothesis is not "the baseline wearing a system's name."
  const coreOpinions = (forecast.appliedAdjustments || []).filter((a) => a.finalAppliedLogOdds > 0);
  const creativeOpinions = useExploration && forecast.exploration ? forecast.exploration.activeHypotheses || 0 : 0;
  const opinions = coreOpinions.length + creativeOpinions ? [1] : [];
  if (!opinions.length) {
    out.noOpinion = true;
    out.classification = "NO BET";
  ...
  // CORE: ceiling is the CONSERVATIVE bound (a max price off the central is not a real limit). READ lane:
  // ceiling is what you BELIEVE it's worth — the read's own central — because the re-point pays up to the
  // read, not up to a market-anchored floor. The tier's central-EV gate still keeps a margin below it.
  out.maximumAcceptablePrice = +((useExploration ? out.systemCentralProbability : conservative)).toFixed(4);
  ...
  // The exploration lane is deliberately allowed a wider bar than core: a CREATIVE SPECULATIVE
  // position may have a favourable CENTRAL EV even if the conservative bound crosses break-even
  // (the config's per-tier thresholds decide). Core is unchanged: it still requires conservative > 0.
  if (useExploration) {
    out.lane = "exploration";
    out.explorationCentralEV = out.expectedValueCentral;
    out.explorationConservativeEV = out.expectedValueConservative;
    // Qualification and tier are decided by the exploration sizer against config/exploration-rules.json.
    // Here we only gate on executability + a non-negative CENTRAL EV (a losing central EV never sizes).
    out.qualifies = out.expectedValueCentral > 0 && ex.fullyFilled;
    out.classification = out.qualifies ? "EXPLORATION CANDIDATE" : "NO BET";
    if (!out.qualifies) out.reason = out.expectedValueCentral <= 0
      ? `creative central EV ${out.expectedValueCentral} is not favourable after fees`
