...
// cap 0.92: the fitted calibration (Brier ~0.14 on real fights) genuinely separates strong picks in the
// 86-93% range; an 0.85 cap clamped them all flat to 85%. 0.92 lets that spread show for parlay math while
// still never asserting near-certainty from a YouTube consensus.
const DEFAULT = { method: "conservative-default", slope: 0.65, coverageK: 3, cap: 0.92, floor: 0.50,
...
function fit(samples, { coverageK = DEFAULT.coverageK } = {}) {
  ...
  const brier = pts.reduce((a, s) => {
    const p = Math.max(DEFAULT.floor, Math.min(DEFAULT.cap, 0.5 + (s.share - 0.5) * slope * coverageFactor(s.coverage || 0, coverageK)));
    return a + (p - s.won) ** 2;
  }, 0) / pts.length;
