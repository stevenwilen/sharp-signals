...
// READ-ONLY AT THE TRANSPORT, not at the export names. assertNoTradingPath() used to prove only that
// no function was *named* createOrder — while this function, exported and generic, signs and POSTs
// whatever body it is handed. `request("POST", "/portfolio/orders", {body, auth})` would have placed a
// real order with the guard still returning true. A deny-list of five names is the same bug as the
// deny-list of one string ("maker"), one level up. The method is the thing that decides whether bytes
// leave as a read or a write, so refuse here: nothing in this repo calls request with anything but GET
// (verified), so this is a structural floor, not a behaviour change.
// Deliberately NOT async: an `async function` that throws returns a rejected promise instead of
// throwing, so a synchronous `try { request("POST", ...) } catch` would sail straight past the refusal
// and leave an unhandled rejection behind. The check has to land before the async boundary or the guard
// is decoration — so validate here and delegate the retry loop to the async worker below.
const READ_METHODS = new Set(["GET", "HEAD"]);

function request(method, path, opts = {}) {
  if (!READ_METHODS.has(String(method).toUpperCase())) {
    throw new Error(
      `kalshi.request: method ${method} refused — this build is read-only and has no order path. ` +
      `If you are deliberately adding one, you are changing what this system IS: see lib/arming.js.`
    );
  }
  return requestWithRetry(method, path, opts);
}
