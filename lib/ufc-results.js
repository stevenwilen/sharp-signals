...
// The method and round cells, which the first version parsed past and discarded. Without them the
// scenario layer cannot be evaluated on method at all — "did the primary path call the finish?" is
// unanswerable, and an unanswerable question quietly becomes a pass.
//
// Row shape: {{yes2}}Win | record | [[Opponent]] | METHOD | [[Event]] | {{dts|...}} | ROUND | time | location
// Cells are newline-pipe separated; pipes INSIDE a cell ({{dts|...}}, "align=center|1") are not,
// which is what makes a naive split on "|" wrong and this split on /\n\s*\|/ right.
const METHOD_CANON = [
  [/^(?:ko|tko)\b/i, "KO/TKO"],
  [/^submission\b/i, "Submission"],
  [/^(?:technical\s+)?submission\b/i, "Submission"],
  [/^decision\b/i, "Decision"],
  [/^(?:dq|disqualification)\b/i, "DQ"],
  [/^(?:technical\s+)?draw\b/i, null],
  [/^(?:nc|no\s+contest)\b/i, null],
];
function methodAndRound(body) {
  const cells = String(body).split(/\n\s*\|/).map((c) =>
    c.replace(/<!--[\s\S]*?-->/g, "")            // editor notes
     .replace(/\{\{\s*small\s*\|[\s\S]*?\}\}/gi, "")
     .replace(/^\s*align\s*=\s*center\s*\|/i, "") // "align=center|1" -> "1"
     .trim()).filter((c) => c !== "");
  // cells[0]=running record, [1]=opponent, [2]=method, [3]=event, [4]=date, [5]=round
  const rawMethod = cells[2] || null;
  let method = null, methodDetail = null;
  if (rawMethod) {
    methodDetail = rawMethod.replace(/\[\[(?:[^\]|]+\|)?([^\]|]+)\]\]/g, "$1").trim();
    for (const [re, canon] of METHOD_CANON) if (re.test(methodDetail)) { method = canon; break; }
  }
  // Round is the first bare integer 1-5 AFTER the date cell. Searching from the start would match
  // the running record ("22–4") or a stray number in the method text.
  let round = null;
  const dateIdx = cells.findIndex((c) => /\{\{\s*dts/i.test(c));
  if (dateIdx >= 0) {
    const r = (cells[dateIdx + 1] || "").replace(/^\s*align\s*=\s*center\s*\|/i, "").trim();
    if (/^[1-5]$/.test(r)) round = Number(r);
  }
  return { method, methodDetail, round };
}

// Parse the {{MMA record}} table -> [{ result:1|0|null, opponent, date, method, methodDetail, round }].
...
function parseRecord(wt) {
  ...
  while ((m = re.exec(wt))) {
    ...
    out.push({ result, opponent: opp ? opp[1].trim() : null, date, ...methodAndRound(body) });
  }
  return out;
}
...
async function resolveOutcome(pick, opponent, match, opts = {}) {
  ...
  if (rp) {
    ...
    // method/round describe the FIGHT, so they carry through unchanged from whichever page the row
    // was read on — unlike `result`, which is relative to the page's owner and inverts below.
    if (row && row.result != null) return { ok: true, result: row.result, source: "wikipedia", via: rp.title, date: row.date,
      method: row.method, methodDetail: row.methodDetail, round: row.round };
  }
  if (opponent) {
