...
// Has the FIGHT DAY arrived? TRUE the moment it becomes the card's calendar date in the operator's
// timezone — for the WHOLE day, before the earliest prelim, whatever hour it starts. Betting stops on the
// DATE, not a time: Kalshi's occurrence_datetime only marks the main card / a later bout (it read 3pm when
// the prelims actually started at 10am), so a time-based lock lets betting run into the live early prelims.
// Default tz America/New_York (UFC is US-based; matches the daily report's REPORT_TZ).
function fightDayStarted(eventDate, now = Date.now(), tz = process.env.REPORT_TZ || "America/New_York") {
  if (!/^\d{4}-\d{2}-\d{2}$/.test(String(eventDate || ""))) return false;
  let localDate;
  try { localDate = new Intl.DateTimeFormat("en-CA", { timeZone: tz, year: "numeric", month: "2-digit", day: "2-digit" }).format(new Date(now)); }
  catch { localDate = new Date(now).toISOString().slice(0, 10); }   // UTC fallback if the tz is unknown
  return localDate >= eventDate;   // YYYY-MM-DD compares lexically == chronologically
}

module.exports = { S, corpusStatus, marketPriceStatus, consensusStatus, historicalStatus, fightStarted, tickerDateFor, cardIsLive, fightDayStarted };
