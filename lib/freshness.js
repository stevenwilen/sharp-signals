...
// Is the card LIVE right now? TRUE the moment ANY bout on it has left the tradeable set — status no
// longer "active"/"open" (i.e. resolving/determined/finalized/settled/closed). This reads REAL Kalshi
// status, so unlike the 22:00 bell it holds no matter when the card actually starts (fight day varies),
// and it locks the WHOLE card once the first result lands — which is exactly the "betting off a market
// moving on fight day" accident. `markets` is the KXUFCFIGHT series list (all statuses). Fail-closed
// inputs (no markets / unparseable date) return false so this only ever ADDS a lock, never removes one.
function cardIsLive(markets, eventDate) {
