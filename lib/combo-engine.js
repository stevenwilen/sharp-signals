...
function evaluateCombo(legs, quote, exposure = {}, opts = {}) {
  ...
  // exchange does not currently offer as a readable market is UNAVAILABLE, never a silent "no bet".
  if (!quote || quote.available === false)
    return { decision: DECISION.UNAVAILABLE, reason: (quote && quote.reason) || "no live combo quote available", audit: { ...audit, quote: quote || null } };
