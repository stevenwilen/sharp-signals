...
// WHO WON, FROM THE LEDGER — so a result stays lookupable after Kalshi forgets it.
//
// Kalshi serves roughly 63 days of settled markets. The calibration scoreboard used to resolve every
// past read against that live list, which meant it could never accumulate more than ~8 cards: each week
// the oldest card silently dropped out of `graded` with no error and no warning, so the record churned
// instead of growing. The ledger already stores every fight's winner permanently (back to 2024), so it
// is the durable answer and Kalshi is only the fallback for something not yet recorded.
//
// Matched on the surname PAIR plus date proximity — the same discipline lib/confidence.js uses, because
// a single surname is not enough (two Millers on one card) and dates drift by a day between sources
// (Kalshi settles after midnight UTC; the historical rows carry the local event date).
function winnerFromLedger(ledger, pick, opponent, aboutMs, surnameOf, surnameEq, withinDays = 3) {
  const ps = surnameOf(pick), os = surnameOf(opponent);
  if (!ps || !os) return null;
  for (const f of Object.values((ledger && ledger.fights) || {})) {
    const t = Date.parse(f.fightTime || 0);
    if (!Number.isFinite(t) || Math.abs(t - aboutMs) > withinDays * 86400e3) continue;
    const as = surnameOf(f.a), bs = surnameOf(f.b);
    if (!as || !bs) continue;
    if (surnameEq(ps, as) && surnameEq(os, bs)) return f.winner === "a" ? 1 : 0;
    if (surnameEq(ps, bs) && surnameEq(os, as)) return f.winner === "b" ? 1 : 0;
  }
  return null;
}

module.exports = { gradeOne, gradeAll, rowsFromLedger, winnerFromLedger, weightOf };
