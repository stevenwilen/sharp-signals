...
//
// MEMOIZED. The closing line and the result are properties of the FIGHT, not of who predicted it —
// so all 50 channels' picks for "McGregor vs Holloway 2026" resolve to the identical answer. Over a
// 34k-pick corpus that same fight is looked up hundreds of times; without this the throttled BFO +
// Wikipedia calls repeat endlessly (a multi-hour grind). Key by canonical fighter+opponent+year so
// spelling variants collapse to ONE lookup while a rematch in a different year stays distinct.
// Negative results are cached too (as null) so a page-less prelim fighter is searched once, not per
// pick. Cleared at the start of each resolveAll run.
let histMemo = new Map();
async function resolveHistorical(pred) {
  if (pred.domain !== "mma" || !pred.opponent) return null;
  const key = `${names.canonical(pred.pick)}|${names.canonical(pred.opponent)}|${String(pred.timestamp || "").slice(0, 4)}`;
  if (histMemo.has(key)) {
