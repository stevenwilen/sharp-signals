...
function load() {
  try {
    ...
    return {
      ...
      prospectiveStartAt: null,          // stamped once, when PAPER mode first opens a position
      rulesetVersion: null,
      positions: {},
      observations: {},
      lastRun: null,
...
// Stamp the official experiment start ONCE, only ever from PAPER mode. OBSERVE never stamps it, so nothing
// observed pre-certification is ever counted as official performance.
function ensurePaperStarted(state, at = nowIso(), rulesetVersion) {
...
function processObservations(state, rawObs, { profile, mode, now = nowIso() }) {
  ...
  for (const key of sortedKeys) {
    ...
    if (mode === MODES.PAPER) {
      ensurePaperStarted(state, now, profile.version);
...
function summary(state, opts = {}) {
  ...
  const realizedPnl = r2(settled.reduce((a, p) => a + (p.pnl || 0), 0));
  const pessimisticRealizedPnl = r2(settled.reduce((a, p) => a + (p.pessimisticPnl || 0), 0));
  ...
  const settledSorted = [...settled].sort((a, b) => String(a.settledAt || "").localeCompare(String(b.settledAt || "")));

  const byCategory = {};
  for (const p of settled) { const k = p.primaryQualification || "UNCATEGORIZED"; (byCategory[k] = byCategory[k] || blankBucket()); accumulate(byCategory[k], p); }
  // co-qualification frequency (not P&L) so one outcome is never counted as multiple bets
  for (const p of settled) for (const c of (p.contributingQualifications || [])) if (c !== p.primaryQualification && byCategory[c]) byCategory[c].coQualified++;
  ...
  const blended = slice(settled, starting);
  const incremental = slice(settled.filter((p) => p.primaryQualification !== "CORE_BUY"), starting);
  ...
  const result = {
    ...
    counts: { total: positions.length, open: open.length, settled: n, wins, losses, voids: n - decided, observations: observations.length },
    sampleWarning,
    perCategory,
...
module.exports = {
  ...
  loadProfile, load, save, ensurePaperStarted,
