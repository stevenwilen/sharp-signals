...
function evaluate(profile, obs) {
  ...
  // STALE-PRIOR refusal (mirrors production's LOGICAL_OPEN gate). The creative central is only a valid
  // prior for a decision made against a CONTEMPORANEOUS price. If the sealed forecast predates the live
  // ask by more than the freshness window, any "edge" is the market's DRIFT since the seal, not skill —
  // which is what let v2 fund both sides of a bout as the price moved. Refuse rather than trade a timing gap.
  {
    const sealMs = Date.parse(obs.signalTimestamp), askMs = Date.parse(obs.marketPriceTimestamp);
    if (Number.isFinite(sealMs) && Number.isFinite(askMs) && (askMs - sealMs) > 6 * 3600e3)
      return fail(out, `forecast prior is ${Math.round((askMs - sealMs) / 3.6e6)}h older than the live price — stale, refusing to trade a timing gap`);
  }
...
function processObservations(state, rawObs, { profile, mode, now = nowIso() }) {
  ...
  for (const key of sortedKeys) {
    ...
    // ONE POSITION PER BOUT. v2 funded BOTH sides of a fight (ALIDAV-ALI and ALIDAV-DAV) as the price
    // drifted across days — a guaranteed loss, never an edge. If any OPEN position sits on the SAME bout
    // (the ticker minus its side suffix, so both sides collapse to one key), do not open another.
    const boutKey = String(market).replace(/-[A-Za-z0-9]+$/, "");
    const heldSameBout = Object.values(state.positions).find((p) => p.status === STATUS.OPEN
      && String(p.ticker || p.market || "").replace(/-[A-Za-z0-9]+$/, "") === boutKey
      && (p.ticker || p.market) !== market);
    if (heldSameBout) {
      for (const m of members) decisions.push(decision(m, DISPOSITION.OBSERVED_NO_ENTRY,
        `already hold ${heldSameBout.ticker || heldSameBout.market} on this bout — one position per bout`, null));
      counts.observedNoEntry++; continue;
    }
    ...
    const posCore = {
      ...
      // NEAR-MISS tracking: how far the live ask was ABOVE the production maximum acceptable price (cents).
      // This is the "we needed it 1c lower" case — captured so its record can be graded over a sample.
      productionMaxAcceptable: primary.m.productionMaximumAcceptablePrice ?? null,
      priceGapCents: (primary.m.category === "UNCONFIRMED_CANDIDATE" && primary.m.productionMaximumAcceptablePrice != null && primary.m.observedAsk != null)
        ? Math.round((primary.m.observedAsk - primary.m.productionMaximumAcceptablePrice) * 100) : null,
...
function summary(state, opts = {}) {
  ...
  // NEAR-MISS slice: the PRICE_TOO_HIGH candidates (the "needed it a cent lower" bets). This is the direct
  // answer to "should we have sent that signal?" — measured over a sample instead of one anecdote.
  const nmSettled = settledO.filter((p) => p.primaryQualification === "UNCONFIRMED_CANDIDATE");
  const nearMiss = slice(nmSettled, starting);
  const nmGaps = nmSettled.map((p) => p.priceGapCents).filter((g) => Number.isFinite(g));
  nearMiss.avgPriceGapCents = nmGaps.length ? r2(nmGaps.reduce((a, b) => a + b, 0) / nmGaps.length) : null;
  nearMiss.openCount = openO.filter((p) => p.primaryQualification === "UNCONFIRMED_CANDIDATE").length;
...
function normalizePosition(p) {
  return {
    ...
    priceGapCents: p.priceGapCents ?? null, productionMaxAcceptable: p.productionMaxAcceptable ?? null,
