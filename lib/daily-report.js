...
//   receipts, forecast, intelligence, entertainment, bankrolls, manualBankroll,
//   candidateIndex, coverage, cardEvidence, alertLedger, geminiUsage, runs, telegramSends }
function buildReport(inp) {
  ...
  const report = {
    ...
    generatedAtMs: now,
    // The two things the operator actually reads on Telegram: is it running, and when's the next fight.
    nextFight: inp.card && inp.card.eventDate
      ? { eventId: inp.card.eventId, eventDate: inp.card.eventDate, firstBellIso: inp.card.bellMs ? new Date(inp.card.bellMs).toISOString() : null }
      : null,
    bankrollNow: bank && bank.real && bank.real.accountValue != null ? round2(bank.real.accountValue) : null,
    recommendationsToday: production.coreBuy,
    sections: { activity, source, picks, fightIntelligence: fi, production, real, quality },
  };
...
const statusGlyph = (s) => (s === "SYSTEM HEALTHY" ? "✅" : s === "SYSTEM DEGRADED" ? "⚠️" : "❌");

function formatTelegram(r) {
  const L = [];
  const p = (line) => L.push(line);
  // Slim STATUS — the operator reads exactly two things here: is it running, and when's the next fight.
  // The FULL section data still lives in the report JSON (the dashboard), it just doesn't crowd the phone.
  p(`🩺 DAILY STATUS — ${r.reportDate}`);
  p(`${statusGlyph(r.status)} ${r.status}`);       // ✅/⚠️/❌ SYSTEM HEALTHY / DEGRADED / FAILED
  if (r.status !== "SYSTEM HEALTHY" && r.verdictReasons.length) p(`⚠ ${r.verdictReasons.slice(0, 3).join("; ")}`);

  if (r.nextFight && r.nextFight.eventDate) {
    const bell = r.nextFight.firstBellIso ? ` · first bell ~${r.nextFight.firstBellIso.slice(11, 16)} UTC` : "";
    p(`🥊 Next fight: ${r.nextFight.eventId || r.nextFight.eventDate} — ${r.nextFight.eventDate}${bell}`);
  } else {
    p("🥊 Next fight: none scheduled yet");
  }

  const recs = r.recommendationsToday || 0;
  p(`💵 Bankroll ${r.bankrollNow == null ? "—" : "$" + r.bankrollNow.toFixed(2)} · ${recs} bet${recs === 1 ? "" : "s"} found today`);
  return L.join("\n");
}
