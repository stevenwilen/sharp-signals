...
//
// `weights` (optional) overrides the channel weight table. Live scoring passes nothing and gets the
// current record off disk; run-fit-calibration passes the table as it stood BEFORE the card, so a past
// card is never rebuilt with weights that already know how it ended.
function scoreBout(bout, picks, weights = null) {
  const infoOf = (src) => (weights ? (weights[src] || { weight: W.UNGRADED_WEIGHT, tier: "UNRANKED", graded: false }) : W.infoFor(src));
...
// opts.weights: see scoreBout — the walk-forward hook, unused on the live path.
function buildCard(eventDate, opts = {}) {
  const bouts = cardBouts(eventDate);
  const gathered = gatherAllPicks(bouts, eventDate);
  const fights = bouts.map((b) => scoreBout(b, gathered[b.boutId], opts.weights || null))
