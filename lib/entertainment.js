...
// Money constants come from ONE canonical file (config/bankroll.json), shared with the exploration
// lane so the two can never silently drift. Same values as before — one home, not a new number.
const MONEY = require("../config/bankroll.json");

const BANKROLL = {
  amount: MONEY.bankrollDollars,
  currency: "USD",
  label: "ENTERTAINMENT",
...
const TIERS = {
  STANDARD: { fraction: MONEY.tiers.tier1.fraction, dollars: MONEY.tiers.tier1.dollars, label: "standard experimental" },
  STRONG: { fraction: MONEY.tiers.tier2.fraction, dollars: MONEY.tiers.tier2.dollars, label: "strong experimental" },
  MAXIMUM: { fraction: MONEY.tiers.tier3.fraction, dollars: MONEY.tiers.tier3.dollars, label: "rare maximum" },
};
const CAPS = {
  maxFractionPerFight: MONEY.maxFractionPerFight,   // 5% = $5
  maxFractionPerCard: MONEY.maxFractionPerCard,     // 10% = $10
