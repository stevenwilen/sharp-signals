...
// ---------------------------------------------------------------------------------------------
// CONTRACT DISCOVERY — which listed markets are genuine FIGHT-OUTCOME markets, and which only look
// like them. Production must evaluate ONLY contracts Kalshi actually returns, and must never treat an
// announcer-mention prop as a fight outcome.
//
// The trap is specific and live. On the SAME card, `KXFIGHTMENTION` uses the IDENTICAL event codes as
// `KXUFCFIGHT` (both carry `26JUL18DUUSM`) and its strikes are named "Knockout", "Decision", "What a
// Fight" — but it resolves on WHAT THE BROADCAST COMMENTATORS SAY, not on who wins. A discovery path
// that keyed on the event code, or on a `/KO|decision|round/` regex over titles, would price a
// commentary prop as a method bet. (That title regex also returns 100% false positives on real
// KXUFCFIGHT markets — Seok Hyun *Ko*, Donchen*ko*.)
//
// So discovery keys on the SERIES and on the resolution language in the rules, never on the title.
// ---------------------------------------------------------------------------------------------

// Series whose markets resolve on the FIGHT OUTCOME. Extensible on purpose: when Kalshi lists method /
// round / distance series and their fee scope is verified, add them here. As of 2026-07-18 a
// cursor-honest probe found none of the commonly-cited method series (KXUFCMOF, KXUFCMOV,
// KXUFCVICROUND, KXUFCROUNDS, KXUFCDISTANCE) listed — 0 open markets each — so production evaluates
// KXUFCFIGHT only. The allowlist is the mechanism; the inventory is whatever Kalshi actually returns.
const FIGHT_OUTCOME_SERIES = ["KXUFCFIGHT", "KXBOXING"];

// Series known to be resolved by something OTHER than the fight outcome. Explicitly named so a future
// discovery expansion cannot pick them up by accident.
const NON_OUTCOME_SERIES = ["KXFIGHTMENTION"];

// The resolution language of an announcer-mention market. Keyed on the RULES text — the source of
// truth for how a market settles — never on the strike title.
const ANNOUNCER_RESOLUTION = /\b(commentator|announcer|broadcast(er)?|play by play|color commentator|sideline reporter)\b[^.]*\bsay/i;
const OUTCOME_RESOLUTION = /\b(wins|win by|defeats|by (ko|tko|submission|decision)|the fight (is|does not) (go|reach)|goes the distance|round\b)/i;

// Classify one Kalshi market. `accept` is true only for a genuine fight-outcome market on an allowed
// series whose rules do not read as announcer-resolved. Anything uncertain is REFUSED, not guessed.
function classifyMarket(m) {
  const series = String(m.ticker || m.event_ticker || "").split("-")[0];
  const rules = String(m.rules_primary || "");

  if (NON_OUTCOME_SERIES.includes(series) || ANNOUNCER_RESOLUTION.test(rules)) {
    return { accept: false, series, kind: "announcer_mention",
      reason: `${series} resolves on what the broadcast says, not on the fight outcome — not a fight-outcome market` };
  }
  if (!FIGHT_OUTCOME_SERIES.includes(series)) {
    return { accept: false, series, kind: "unknown_series",
      reason: `series ${series} is not on the fight-outcome allowlist — refusing rather than assuming it resolves on the result` };
  }
  if (rules && !OUTCOME_RESOLUTION.test(rules)) {
    return { accept: false, series, kind: "unrecognised_resolution",
      reason: `${series} market's rules do not read as a fight-outcome resolution — refusing to classify it` };
  }
  return { accept: true, series, kind: "fight_outcome", reason: "resolves on the fight outcome" };
}

const isAnnouncerMentionMarket = (m) => classifyMarket(m).kind === "announcer_mention";

// Filter a list of listed markets to the genuine fight-outcome contracts. Returns { admitted, rejected }
// so a caller can LOG what it dropped — a silently-filtered board reads as "nothing there" when it
// might have been "everything refused".
function admissibleFightMarkets(markets) {
  const admitted = [], rejected = [];
  for (const m of markets || []) {
    const c = classifyMarket(m);
    (c.accept ? admitted : rejected).push({ ticker: m.ticker, ...c });
  }
...
module.exports = {
  ...
  classifyMarket, isAnnouncerMentionMarket, admissibleFightMarkets,
  FIGHT_OUTCOME_SERIES, NON_OUTCOME_SERIES,
};
