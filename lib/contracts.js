...
// transcribed from Kalshi's published schedule and then CONFIRMED on 2026-07-16 against four
...
const FEES = {
  source: "Kalshi published taker fee schedule, CONFIRMED against four authenticated UFC Quick Order tickets (2026-07-16): $100 x3 at prices 0.59/0.69/0.89 and $500 x1 at 0.59",
  ...
  verifiedScope: {
    ...
    // Widened from [111.49, 164.76] by the $500 ticket, which is the ONLY thing that licenses it:
    // it holds price fixed at 0.59 and varies size alone.
    sizeRange: [111.49, 823.81],
    notionalRange: ["$100", "$500"],
    fills: "single-price fills only",
    rateConstrainedTo: [0.069952, 0.070002],
    establishes:
      "taker rate 0.07 and ceil-to-cent rounding, for single-price YES fills on KXUFCFIGHT as of 2026-07-16. " +
      "Four authenticated Quick Order tickets reproduce exactly (diff 0.00). The admissible rate interval under " +
      "ceil is (0.069952, 0.070002] — 50ppm wide, containing 0.07 as its only two-decimal value. ceil is " +
      "discriminated: floor fits 0/4, round-half-up fits 1/4. LINEARITY IN CONTRACTS IS MEASURED, not assumed: " +
      "the $500 ticket holds price fixed at 0.59 and varies size alone, 164.76 -> 823.81 contracts (5.0001x) " +
      "producing 2.79 -> 13.95 (5.0000x), which refutes every rival exponent k*gross^x for x in 0.8-1.5.",
    doesNotEstablish: [
      ...
      "Sizes below 111.49 contracts, where ceil dominates: 1 contract at 0.50 costs $0.02 = 4.0% of notional vs 3.5% asymptotically. NOTE: with the current 0.5%-of-bankroll cap, a $10k bankroll proposes roughly 85 contracts — BELOW this floor — so real positions currently sit outside the verified band and will carry the extrapolation caveat.",
    ...
    examples: "data/fee-examples.json — four unsubmitted Kalshi UFC Quick Order tickets ($100 x3, $500 x1)",
...
// Is this order inside the band the four tickets actually verified? Anything outside keeps the
// extrapolation caveat, which is the whole point of a scoped record over a boolean.
function withinVerifiedEnvelope(contract, side, contracts, price, cfg = FEES) {
  ...
  if (Number.isFinite(contracts) && (contracts < s.sizeRange[0] || contracts > s.sizeRange[1]))
    out.push(`size ${contracts} is outside the verified band ${s.sizeRange[0]}-${s.sizeRange[1]} contracts ($100-$500 tickets)`);
...
function priceOrder(contract, orderbook, contracts, opts = {}) {
  ...
  // The four tickets verified single-price YES fills on KXUFCFIGHT at 0.59-0.89 and 111-824
