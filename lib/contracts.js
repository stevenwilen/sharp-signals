...
// transcribed from Kalshi's published schedule and then CONFIRMED on 2026-07-16 against five
...
const FEES = {
  source: "Kalshi published taker fee schedule, CONFIRMED against five authenticated UFC Quick Order tickets (2026-07-16): $100 x3 at prices 0.59/0.69/0.89, plus $50 and $500 at 0.59 which vary size at a fixed price",
  ...
  verifiedScope: {
    ...
    // The size band is bounded by two tickets that hold price fixed at 0.59 and vary size alone —
    // the $50 ticket sets the floor, the $500 ticket the ceiling. Nothing else licenses this range.
    sizeRange: [82.37, 823.81],
    notionalRange: ["$50", "$500"],
    ...
    establishes:
      ...
      "Five authenticated Quick Order tickets reproduce exactly (diff 0.00). The admissible rate interval under " +
      "ceil is (0.06995195, 0.07000213] — 50ppm wide, containing 0.07 as its only two-decimal value. ceil is " +
      "discriminated: floor fits 0/5, round-half-up fits 2/5 — it reproduces only the two tickets whose exact " +
      "cents happen to land above the .5 boundary. LINEARITY IN CONTRACTS IS MEASURED over a 10x " +
      "span at a fixed price, not assumed: at 0.59, 82.37 -> 164.76 -> 823.81 contracts (10.0013x end to end) " +
      "produce exact pre-ceil fees 1.394771 -> 2.789881 -> 13.949575 (10.0013x), refuting every rival exponent " +
      "k*gross^x for x in 0.8-1.5. The ceil premium is observed rather than inferred: it adds 0.375% of the fee " +
      "at 82.37 contracts and 0.003% at 823.81.",
    doesNotEstablish: [
      ...
      "Sizes below 82.37 contracts, where the ceil premium grows without bound as size shrinks: 1 contract at 0.50 costs $0.02 = 4.0% of notional vs 3.5% asymptotically. The premium is measured at the 82.37 floor (0.375% of the fee) but nothing below it is tested. NOTE: at a $5k bankroll the 0.5% cap proposes ~42 contracts, still below this floor and still correctly caveated; a $10k bankroll proposes ~85, which is inside.",
    ...
    examples: "data/fee-examples.json — five unsubmitted Kalshi UFC Quick Order tickets ($50 x1, $100 x3, $500 x1)",
...
// Is this order inside the band the five tickets actually verified?
//
...
function withinVerifiedEnvelope(order, cfg = FEES) {
  ...
  if (!Number.isFinite(contracts)) out.push("no size supplied");
    ...
    out.push(`size ${contracts} is outside the verified band ${s.sizeRange[0]}-${s.sizeRange[1]} contracts ($50-$500 tickets)`);
...
function priceOrder(contract, orderbook, contracts, opts = {}) {
  ...
  // The five tickets verified single-price YES fills on KXUFCFIGHT at 0.59-0.89 and 82.37-823.81
