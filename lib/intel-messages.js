...
function shouldAlert(prev, record, ctx = {}) {
  ...
  // §4: an analytical hypothesis / public history / low-value narrative is forecast-lane research, not
  // breaking news. It reaches the phone ONLY when it becomes an actual bet or resolves — its WATCH and
  // FORECAST_UPDATED states live on the dashboard. Event-status and current-condition reports are news.
  const newsy = record.reportType === I.REPORT_TYPE.EVENT_STATUS || record.reportType === I.REPORT_TYPE.CURRENT_CONDITION;
  const betLifecycle = [A.SPECULATIVE_BET, A.MARKET_ALREADY_MOVED, A.POSITION_WITHDRAWN, A.REPORT_CONFIRMED, A.REPORT_DISPROVED, A.HUMAN_ACTION_REQUIRED].includes(record.actionStatus);
  if (!newsy && !betLifecycle) return { alert: false, reason: "analytical / forecast-lane — dashboard only, not phone news" };
  if (!prev) return { alert: true, reason: "first material sighting" };
  // A material record that has NEVER been sent must alert once — this is how records discovered in
  // shadow are delivered the first time send is switched on, without re-blasting already-sent ones.
  if (!(record.telegramLineage && record.telegramLineage.length)) return { alert: true, reason: "material and not yet sent (e.g. promoted from shadow)" };
