...
//
// opts.replyTo threads a later update UNDER its original alert (§7): pass either a single message id,
// or a { chatId -> messageId } map so each recipient's update replies to the message THAT recipient got.
// The return now carries `messages: [{ chatId, messageId }]` — the message ids are captured (they used
// to be discarded), so an intelligence record can store its Telegram lineage and thread its follow-ups.
async function notify(text, opts = {}) {
  if (!TOKEN()) return { skipped: "no TELEGRAM_BOT_TOKEN", messages: [] };
  let ids = String(process.env.TELEGRAM_CHAT_ID || "").split(",").map((s) => s.trim()).filter(Boolean);
  if (!ids.length) {
    const found = await discoverChatId();
    if (found) ids = [found];
  }
  if (!ids.length) return { error: "no chat id — message the bot once, then retry", messages: [] };
  const sent = [], failed = [], messages = [];
  for (const id of ids) {
    const params = { chat_id: id, text: String(text).slice(0, 3900) };
    const replyId = opts.replyTo && (typeof opts.replyTo === "object" ? opts.replyTo[id] : opts.replyTo);
    if (replyId) params.reply_to_message_id = replyId;
    // Call through module.exports.api so tests can stub the transport (no network in the offline suite).
    const j = await module.exports.api("sendMessage", params);
    if (j.ok) { sent.push(id); if (j.result && j.result.message_id) messages.push({ chatId: id, messageId: j.result.message_id }); }
    else failed.push(id);
  }
  return failed.length ? { ok: sent.length > 0, sent, failed, messages } : { ok: true, sent, messages };
}
