...
async function findVideos(sources, sinceIso, log = () => {}) {
  ...
  for (const s of sources) {
    ...
    try {
      // fighters/coaches get the wide net (they don't title videos "predictions")
      const insider = ["fighter", "coach"].includes(s.type);
      const vids = await predictionVideos(s.handle, sinceIso, { insider, cap: insider ? 12 : 20 });
      if (vids === null) { log(`  ${s.name}: channel not found (check the handle)`); continue; }
      log(`  ${s.name}: ${vids.length} videos${insider ? " (insider: wide net)" : ""}`);
