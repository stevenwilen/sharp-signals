...
const SPORT_RE = /(ufc|mma|fight|fighter|bout|octagon|knockout|\bko\b|main event|undercard|welterweight|middleweight|heavyweight|lightweight|featherweight|bantamweight|flyweight)/i;
...
// Plain HTTPS GET (follows one redirect) for the key-free discovery path — the RSS feed and, on a
// cache miss, the channel page. A browser-ish UA keeps YouTube from serving a consent interstitial.
function httpsGet(url, { timeout = 20000, redirects = 3 } = {}) {
  return new Promise((resolve, reject) => {
    https.get(url, { timeout, headers: { "user-agent": "Mozilla/5.0 (compatible; SharpSignals/1.0)", "accept-language": "en-US,en" } }, (r) => {
      if (r.statusCode >= 300 && r.statusCode < 400 && r.headers.location && redirects > 0) {
        r.resume(); return resolve(httpsGet(r.headers.location, { timeout, redirects: redirects - 1 }));
      }
...
// @handle -> { channelId }.  KEY-FREE: the id is immutable so it is CACHED (data/channels.json); on a
// miss we scrape the public channel page for its "channelId":"UC…" — no YouTube Data API key. Existing
// cache entries also carry a legacy `uploads` playlist id, which the RSS path no longer needs.
async function resolveChannel(handle) {
  const h = String(handle).replace(/^@/, "");
  const cache = loadCh();
  if (cache[h] && cache[h].channelId) return cache[h];
  let body;
  try { ({ body } = await httpsGet(`https://www.youtube.com/@${encodeURIComponent(h)}`)); }
  catch (_) { return null; }             // transient — a fixed handle re-resolves next run
  const m = body.match(/"channelId":"(UC[\w-]{20,})"/) || body.match(/"externalId":"(UC[\w-]{20,})"/)
    || body.match(/youtube\.com\/channel\/(UC[\w-]{20,})/);
  if (!m) return null;                   // not cached — no id found, re-resolve next run
  const resolved = { channelId: m[1] };
  cache[h] = resolved;
  saveCh();
  return resolved;
}

// KEY-FREE via the channel's public RSS feed (no Data API, no quota, no key). The feed returns the
// ~15 most recent uploads, which comfortably covers a card's fight-week window (channels post their
// prediction days before). It does NOT page deep history — that only mattered for the offline backfill
// tool, not for reading the upcoming card. Parsed from the stable <entry> XML with a light regex.
async function recentUploads(channelId, sinceIso, _opts = {}) {
  const since = Date.parse(sinceIso);
  if (!channelId) return [];
  let body;
  try { ({ body } = await httpsGet(`https://www.youtube.com/feeds/videos.xml?channel_id=${encodeURIComponent(channelId)}`)); }
  catch (_) { return []; }
  const out = [];
  for (const e of body.split("<entry>").slice(1)) {   // slice(1) drops the feed header (channel title/date)
    const vid = (e.match(/<yt:videoId>([\w-]{6,})<\/yt:videoId>/) || [])[1];
    const at = (e.match(/<published>([^<]+)<\/published>/) || [])[1];
    const title = (e.match(/<title>([^<]*)<\/title>/) || [])[1] || "";
    if (!vid || !at) continue;
    if (Number.isFinite(since) && Date.parse(at) < since) continue;   // older than the window
    out.push({ videoId: vid, title: decodeXml(title), publishedAt: at });
  }
  return out;
}
...
async function predictionVideos(handle, sinceIso, { insider = false, cap = 0, maxPages = 12 } = {}) {
  ...
  if (!ch) return null;
  const vids = await recentUploads(ch.channelId, sinceIso, { maxPages });
...
async function searchVideos(query, sinceIso, { maxResults = 50 } = {}) {
  // Coverage search is the ONE path still on the paid Data API (search.list has no key-free equivalent).
  // The roster scan is now key-free (RSS); this opt-in extra just goes UNAVAILABLE without a key instead
  // of crashing. COVERAGE_SEARCH_ENABLED is off by default, so this is dormant in production anyway.
  if (!key()) return [];
