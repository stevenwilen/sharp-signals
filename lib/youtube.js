...
async function predictionVideos(handle, sinceIso) {
  const ch = await resolveChannel(handle);
  if (!ch) return null;
  const vids = await recentUploads(ch.uploads, sinceIso);
  return vids
    .filter((v) => PRED_RE.test(v.title) && !POST_RE.test(v.title))
...
async function findVideos(sources, sinceIso, log = () => {}) {
  ...
  for (const s of sources) {
    ...
    try {
      const vids = await predictionVideos(s.handle, sinceIso);
      if (vids === null) { log(`  ${s.name}: channel not found (check the handle)`); continue; }
      log(`  ${s.name}: ${vids.length} prediction videos`);
