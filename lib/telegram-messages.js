...
// The system is PULLING a recommendation it previously issued. Names the fighter + the price it was
// recommended at (a stand-down has to say WHICH bet, in human terms). Crucially it is NOT "cash out": the
// headline said "BET WITHDRAWN", which reads as "withdraw your money" — so a holder would sell and eat the
// entry + exit fees for nothing. If you already placed it, holding to resolution is almost always right.
function positionWithdrawn(w) {
  const name = w.recommendedFirst || w.fight;
  const cents = (w.price != null && Number(w.price) > 0) ? `${Math.round(Number(w.price) * 100)}¢` : null;
  const headline = cents ? `${name} — was buy at ${cents}` : name;
  const t = [
    ...
    "",
    `Reason: ${w.reason}`,
    "Haven't placed it? Skip it.",
    "Already placed? Hold to resolution — don't sell over this, you'd just lose the fees.",
...
// Fine cents: up to one decimal, no trailing ".0" (21.6¢, but 22¢ when whole). Used where a whole-cent
// round would HIDE a sub-cent gap — e.g. a live ask 22¢ vs a fee-adjusted ceiling 21.6¢ both round to
// "22¢", which made a correct PRICE-TOO-HIGH read as the nonsensical "22¢ is above the maximum 22¢".
const centsFine = (x) => x == null ? "n/a" : `${Math.round(x * 1000) / 10}¢`;
...
function priceTooHigh(b) {
  const max = b.maximumAcceptablePrice;
  // The highest WHOLE cent at or below the ceiling — the actual tradeable target ("wait for 21¢", never
  // "wait for 22¢" which was the very price being refused). Floor, so the target is genuinely ≤ ceiling.
  const target = max == null ? null : Math.floor(max * 100 + 1e-9);
  const L = [];
  L.push("⏸️ PRICE TOO HIGH — DO NOT BUY");
  L.push("");
  L.push(b.recommendedFirst || b.fight);
  // "Ask now" not "Current": this is the live ASK — what you'd PAY to buy — which sits above the mid/last
  // shown on the Kalshi ticker, so 22¢ here vs 21¢ on the board is the spread, not a bad read.
  L.push(`Ask now: ${c(b.ask)}`);
  L.push(`Your ceiling: ${centsFine(max)}`);   // fine precision so a sub-cent gap is visible, not rounded away
  L.push("");
  L.push(`Wait for ${target == null ? "n/a" : target + "¢"} or lower. The system will alert again if it becomes available.`);
  const text = L.join("\n");
  assertNoConfidenceScore(text);
  return text;
}
...
// ---- COMBO messages -------------------------------------------------------------------------
// A combo is a SEPARATE market and its message says so. Like the single buy, a COMBO BUY may render
// only after the combo engine reached a COMBO_BUY decision (its own probability/price/fee/correlation/
// bankroll gates all passed) — the price ceiling is guaranteed below fair value before we get here.
function comboBuy(b) {
  const L = [];
  L.push("🎯 COMBO BUY");
  L.push("");
  L.push("Legs:");
  for (const leg of b.legs) L.push(`• ${leg}`);
  L.push("");
  L.push(`Live quote: ${c(b.quote)}`);
  L.push(`Estimated fair price: ${c(b.fairPrice)}`);
  L.push(`MAX BUY PRICE: ${c(b.maxBuyPrice)}`);
  L.push(`Recommended stake: $${Number(b.stake).toFixed(0)}`);
  L.push(`Estimated contracts: ${b.contracts}`);
  L.push(`Maximum payout: $${Number(b.maxPayout).toFixed(0)}`);
  L.push(`Estimated profit after cost and fees: $${Number(b.estProfit).toFixed(2)}`);
  L.push(`Correlation assessment: ${b.correlation}`);
  if (b.whyOne) L.push(`Reason it qualifies: ${b.whyOne}`);
  L.push("");
  if (b.existingSingles && b.existingSingles.length) {
    L.push("Existing singles:");
    for (const s of b.existingSingles) L.push(`• ${s}`);
    L.push("");
  }
  L.push(`Total card exposure after combo: $${Number(b.totalCardExposure).toFixed(0)} / $${b.cardCap}`);
  L.push(`Place only if the displayed average price is ${c(b.maxBuyPrice)} or less.`);
  L.push("");
  L.push("💵 MANUAL RECOMMENDATION — NOT CONFIRMED");
  L.push(FOOTER);
  const text = L.join("\n");
  assertNoConfidenceScore(text);
  return text;
}
function comboPriceTooHigh(b) {
  const L = [];
  L.push("⏸️ COMBO PRICE TOO HIGH");
  L.push("");
  L.push("Legs:");
  for (const leg of b.legs) L.push(`• ${leg}`);
  L.push("");
  L.push(`Live quote: ${c(b.quote)}`);
  L.push(`MAX BUY PRICE: ${c(b.maxBuyPrice)}`);
  L.push(`Required improvement: ${Math.round((b.requiredImprovement || 0) * 100)}¢`);
  L.push("");
  L.push("Keep the existing single positions. Do not buy the combo above the maximum price.");
  L.push(FOOTER);
  const text = L.join("\n");
  assertNoConfidenceScore(text);
  return text;
}
function noComboBet(b) {
  const L = [];
  L.push("➖ NO COMBO BET");
  L.push("");
  L.push("The active individual recommendations remain valid, but their combined market does not offer enough value at a safe price.");
  if (b && b.reason) L.push(`(${b.reason})`);
  L.push(FOOTER);
  const text = L.join("\n");
  assertNoConfidenceScore(text);
  return text;
}
function comboUnavailable(b) {
  const L = [];
  L.push("🚫 COMBO UNAVAILABLE");
  L.push("");
  if (b && b.legs && b.legs.length) { L.push("Legs:"); for (const leg of b.legs) L.push(`• ${leg}`); L.push(""); }
  L.push(b && b.reason ? b.reason : "Kalshi is not currently offering a readable quote for this combination.");
  L.push("");
  L.push("Keep the existing single positions.");
  L.push(FOOTER);
  const text = L.join("\n");
  assertNoConfidenceScore(text);
  return text;
}
function comboWithdrawn(b) {
  const L = [];
  L.push("❌ COMBO WITHDRAWN");
  L.push("");
  if (b && b.legs && b.legs.length) { L.push("Legs:"); for (const leg of b.legs) L.push(`• ${leg}`); L.push(""); }
  L.push(`Reason: ${(b && b.reason) || "the combo no longer qualifies"}`);
  L.push("Do not place the previous combo recommendation.");
  L.push(FOOTER);
  const text = L.join("\n");
  assertNoConfidenceScore(text);
  return text;
...
module.exports = {
  ...
  comboBuy, comboPriceTooHigh, noComboBet, comboUnavailable, comboWithdrawn,
