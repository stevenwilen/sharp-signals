...
// A bet you placed is OFF — its contract left the board (fight cancelled / rescheduled, or a fighter is
// out). This is the ONE stand-down worth acting on, so it speaks as a SELL. Names the fighter + the price
// it was bought at. Mere drift (price/forecast wiggle while the contract is still listed) NEVER reaches
// here — that stays silent and the bet rides to resolution. Kalshi settles a genuine void at a fair price,
// so if the market has already closed there is nothing lost.
function sellAlert(w) {
  const name = w.recommendedFirst || w.fight;
  const cents = (w.price != null && Number(w.price) > 0) ? `${Math.round(Number(w.price) * 100)}¢` : null;
  const headline = cents ? `${name} — bought at ${cents}` : name;
  const t = [
    "🔴 SELL — the fight is off",
    "",
    headline,
    "",
    `Reason: ${w.reason}`,
    "Sell on Kalshi if the market is still open. If it isn't, Kalshi settles it at a fair price — you don't lose over this.",
...
module.exports = {
  ...
  experimentalPosition, priceUpdate, evidenceUpdate, sellAlert, positionWithdrawn: sellAlert,
