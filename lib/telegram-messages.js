...
function buyInstruction(b) {
  const L = [];
  L.push("🧪 EXPERIMENTAL — MANUAL PLACEMENT ONLY");
  L.push("");
  L.push(b.fight);
  L.push("");
  L.push(`BUY: ${b.contractWording}`);
  L.push(`Ticker: ${b.ticker}`);
  L.push(`Side: YES`);
  L.push("");
  L.push(`Executable price now: ${cents(b.ask)}`);
  L.push(`Maximum acceptable price: ${cents(b.maximumAcceptablePrice)}  ← do not pay more`);
  L.push("");
  L.push(`Stake: ${b.percentOfBankroll}% of your $${b.bankroll} entertainment bankroll = $${b.stake.toFixed(2)}`);
  L.push(`Approximate contracts: ${b.contracts}`);
  L.push(`Tier: ${b.tierLabel} (entertainment sizing — NOT Kelly, NOT sized from a measured edge)`);
  L.push("");
  L.push(`Ranked #1 of ${b.contractsCompared} contracts Kalshi lists on this fight.`);
  L.push(`Why it ranks above the alternatives: ${b.whyTopRanked}`);
  L.push("");
  L.push("Why it qualifies:");
  for (const line of bullets(b.why)) L.push(line);
  L.push("");
  L.push("Main counterargument:");
  for (const line of bullets(b.against)) L.push(line);
  L.push("");
  L.push("DO NOT place it if:");
  for (const line of bullets(b.doNotPlaceIf)) L.push(line);
  L.push("");
  L.push(`System range: ${pct(b.rangeLow)}–${pct(b.rangeHigh)}`);
  L.push(`Conservative value after fees: ${b.conservativeValuePoints >= 0 ? "+" : ""}${b.conservativeValuePoints.toFixed(1)} pts`);
  L.push(`Evidence coverage: ${b.evidenceCoverage}`);
  L.push(`Model status: ${b.modelStatus}`);
  L.push(`Snapshot: ${b.snapshotTimestamp}`);
  if (b.feeGate && !b.feeGate.withinVerifiedEnvelope) {
    L.push("");
    L.push("⚠ FEE IS EXTRAPOLATED — this order is outside the verified fee envelope:");
    for (const e of b.feeGate.exceptions) L.push(`  • ${e}`);
  }
  L.push("");
  L.push("YOU MUST SUBMIT THIS MANUALLY on Kalshi.");
  L.push("This system has no order path and cannot place, modify or close anything.");
  L.push("");
  L.push("📌 STATUS: RECOMMENDATION — NOT CONFIRMED. Real stake $0, excluded from your bankroll P&L");
  L.push(`   until you confirm you placed it:  node run-confirm-placement.js confirm ${b.ticker || "<ticker>"} --price=<fill> --stake=<$>`);
  L.push("");
  // The standing warning. On EVERY alert, without exception, because it is the truest sentence
  // available about this system and the one a run of wins would most tempt a reader to forget.
  L.push("⚠ This system has NOT demonstrated a predictive edge. The only evaluation that ever");
  L.push("  showed one was void (contaminated baseline). Treat this as entertainment.");
  const text = L.join("\n");
  assertNoConfidenceScore(text);
  return text;
}
...
function humanReview(r) {
  const L = [];
  L.push("🔎 HUMAN REVIEW — UNVERIFIED");
  L.push("");
  L.push(r.fight);
  L.push("");
  L.push(`Reported about ${r.about}: ${r.claim}`);
  L.push("");
  L.push(`What kind of report: ${r.why}`);
  // The origin count is the load-bearing number in this alert, so it must never read as a bare
  // "unknown" that a reader could mistake for "we don't know how many people know this". Either the
  // real count, or an explicit statement that it was not computed for this item.
  const originsLine = Number.isFinite(r.origins)
    ? `${r.origins}${r.origins === 1 ? " — a single source said this, nobody corroborated it"
        : r.origins >= 2 ? " independent origin(s) — see the count, not the number of channels" : ""}`
    : "not available for this item (no origin count was computed)";
  L.push(`Independent origins: ${originsLine}`);
  L.push(`Topic: ${r.topic}`);
  L.push(`Heard in: ${r.source}`);
  L.push("");
  L.push("THIS IS UNVERIFIED INFORMATION.");
  L.push("It is one report picked out of a transcript. It has not been checked against the UFC,");
  L.push("the promotion, or any official source, and it may simply be wrong or already out of date.");
  L.push("");
  L.push("This is NOT an instruction and NOT a forecast.");
  L.push(`The sealed forecast did NOT move on this: ${r.forecastEffect}`);
  L.push("");
  L.push("If you want to act on it, verify it yourself first.");
  const text = L.join("\n");
  assertNoConfidenceScore(text);
...
module.exports = {
  ...
  humanReview, assertNotABettingInstruction, BETTING_WORDS,
