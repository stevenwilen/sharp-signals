...
//   node run-confirm-placement.js confirm <ticker|key> --price=0.61 --stake=3 [--contracts=5]
...
// actual stake and the price you actually got. Nothing else can move money into the real P&L.
require("./lib/env");
...
function main() {
  ...
  if (cmd === "confirm") {
    ...
    let p;
    try { p = MB.confirmPlacement(state, target, { executionPrice: price, actualStake: stake, actualContracts: contracts }); }
    catch (e) { fail(e.message); }
    MB.save(state);
    say(`✅ MANUALLY PLACED: ${p.fight || p.ticker}`);
    say(`   actual stake $${p.actualStake} @ ${(p.executionPrice * 100).toFixed(1)}c${p.actualContracts ? ` (${p.actualContracts} contracts)` : ""} — now included in your real $${state.bankroll} bankroll P&L.`);
    return 0;
  }
  ...
  fail(`unknown command "${cmd}" — use list | confirm | decline | settle`);
}
