...
(async () => {
  ...
  try {
    ...
    // FIGHT-OVER guard (fight day VARIES — use Kalshi's real per-bout status, not the hardcoded 22:00 bell
    // that let a 15:19 prelim still look "upcoming"). A market no longer "active"/"open" (determined /
    // finalized / settled / closed) has started or resolved, so postBell := true and the eligibility gate
    ...
    } catch (e) { notes.push(`fight-over status check: ${e.message}`); }
