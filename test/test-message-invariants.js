...
{
  const t = TM.sellAlert({ recommendedFirst: "Dricus Du Plessis vs Kamaru Usman", reason: "the bout was rescheduled" });
  ok("headlines SELL — the fight is off", /^🔴 SELL — the fight is off$/m.test(t));
  ok("names the fight", /Dricus Du Plessis vs Kamaru Usman/.test(t));
  ok("gives a reason", /Reason: the bout was rescheduled/.test(t));
  ok("says sell if the market is open, else Kalshi settles fair", /Sell on Kalshi/.test(t) && /fair price/.test(t));
