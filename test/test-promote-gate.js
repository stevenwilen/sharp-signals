// PROMOTION GATE â€” refusal-first. A channel is promoted ONLY on a real, positive-lower-bound graded track
...
const S = (source, over = {}) => ({ source, platform: "youtube", domain: "mma", type: "analyst", handle: "@" + source, n: 20, edgeLcb: 0.05, edge: 0.1, hitRate: 0.6, ...over });

const roster = new Set(["Roster Guy"]);

// 1. A non-roster channel with a real sample + positive edgeLcb is promotable.
{
  const out = selectPromotable({ a: S("Discovered Winner") }, roster);
  ok(out.length === 1 && out[0].source === "Discovered Winner", "1. non-roster, n>=10, edgeLcb>0 -> promotable");
}
...
// 4. Negative-edge REFUSED (edgeLcb <= 0) â€” the common case, correctly rejected.
ok(selectPromotable({ a: S("Loser", { edgeLcb: -0.174 }) }, roster).length === 0, "4. edgeLcb <= 0 is REFUSED (no proven edge)");
ok(selectPromotable({ a: S("Breakeven", { edgeLcb: 0 }) }, roster).length === 0, "4b. edgeLcb exactly 0 is REFUSED (strict >)");
// 5. Non-YouTube REFUSED.
ok(selectPromotable({ a: S("Tweeter", { platform: "x" }) }, roster).length === 0, "5. non-YouTube platform is REFUSED");
// 6. Sorted by edgeLcb descending (best first).
{
  const out = selectPromotable({ a: S("Good", { edgeLcb: 0.03 }), b: S("Better", { edgeLcb: 0.09 }), c: S("Ok", { edgeLcb: 0.01 }) }, roster);
  ok(out.map((x) => x.source).join(",") === "Better,Good,Ok", "6. promotable list sorted by edgeLcb desc");
}
