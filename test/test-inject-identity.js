...
const block = (o = {}) => ([{
  boutId: "UFC-2026-07-18-B01", about: "Kamaru Usman", opponent: "Dricus Du Plessis",
  claim: "Usman's knees are damaged.", topic: "injury_health", direction: "against_about",
  verdict: "CONFIRMED",
  sources: [
    { outlet: "MMA Fighting", origin: "okamoto", url: "https://example.com/a", quote: "His knees are shot.", publishedAt: "2026-07-14T10:00:00Z" },
    { outlet: "ESPN", origin: "helwani", url: "https://example.com/b", quote: "Usman has knee trouble.", publishedAt: "2026-07-14T11:00:00Z" },
  ],
  ...o,
}]);
...
{
  // The exact shipped mis-bind. B03 on this card is Mitch Ramirez vs Chase Hooper.
  const out = run(block({ boutId: "UFC-2026-07-18-B03" }));
  ok("Usman on the Ramirez/Hooper bout is refused", /IDENTITY REFUSED/.test(out));
  ok("...and names the fight it actually is", /Mitch Ramirez vs Chase Hooper/.test(out));
...
{
  const out = run(block());
  ok("Usman on the Usman bout is accepted", !/IDENTITY REFUSED/.test(out), out.split("\n").filter((l) => /REFUS/.test(l)).join("|"));
  ok("...and confirms the side", /confirmed on side a/.test(out));
  ok("...and is admissible", /1 of 1 block\(s\) carry evidence/.test(out));
  const opp = run(block({ about: "Dricus Du Plessis", opponent: "Kamaru Usman" }));
...
{
  const out = run(block({ boutId: "UFC-2026-07-18-B99" }));
...
{
  // Kevin Holland withdrew; his bout was removed from the card entirely. A block about him has nowhere
  // to land, and the positional id will happily point it at somebody else's fight.
  const out = run(block({ about: "Kevin Holland", opponent: "Jacobe Smith" }));
  ok("Kevin Holland (not on this card) is refused", /IDENTITY REFUSED/.test(out));
...
console.log("\nTHE TALLY CANNOT CLAIM AN INJECTION THAT DID NOT HAPPEN");
{
  ...
  const out = run([...block(), ...block({ boutId: "UFC-2026-07-18-B03" })]);
